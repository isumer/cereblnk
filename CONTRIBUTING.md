...
- **On Windows, set `git config core.fileMode false` in your clone.**
  Without it a checkout can clear the POSIX mode bit and a commit will
  carry the loss. Hooks are invoked as commands; one that is not
  executable never fires and never says so. `scripts/verify` runs
  `check-exec-bit` for exactly this, so the loss fails the build rather
